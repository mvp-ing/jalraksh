...
import { connect as jalrakshakGlConnect } from './connect/jalrakshak-connect';
import { IntlProvider } from 'react-intl';
import { messages } from '@jalrakshak/localization';
...
import { generateHashId } from '@jalrakshak/common-utils';

import { shouldForwardProp } from './common/styled-components';

type JalrakshakActions = {
...
} from '@jalrakshak/utils';

import { theme as basicTheme, themeLT, themeBS, breakPointValues } from '@jalrakshak/styles';
...
export const mapFieldsSelector = (props: JalrakshakGLProps, index = 0) => ({
  getMapboxRef: props.getMapboxRef,
  mapboxApiAccessToken: props.mapboxApiAccessToken,
  mapboxApiUrl: props.mapboxApiUrl ? props.mapboxApiUrl : DEFAULT_JALRAKSHAK_PROPS.mapboxApiUrl,
  ...
  sidePanelWidth: props.sidePanelWidth ? props.sidePanelWidth : DEFAULT_JALRAKSHAK_PROPS.width,
...
export const sidePanelSelector = (props: JalrakshakGLProps, availableProviders, filteredDatasets) => ({
  appName: props.appName ? props.appName : DEFAULT_JALRAKSHAK_PROPS.appName,
  version: props.version ? props.version : DEFAULT_JALRAKSHAK_PROPS.version,
  ...
  width: props.sidePanelWidth ? props.sidePanelWidth : DEFAULT_JALRAKSHAK_PROPS.width,
  availableProviders,
  mapSaved: props.providerState.mapSaved
});

export const plotContainerSelector = (props: JalrakshakGLProps) => ({
  width: props.width,
  height: props.height,
...
export const isSplitSelector = (props: JalrakshakGLProps) =>
  props.visState.splitMaps && props.visState.splitMaps.length > 1;

export const bottomWidgetSelector = (props: JalrakshakGLProps, theme) => ({
  filters: props.visState.filters,
...
export const modalContainerSelector = (props: JalrakshakGLProps, rootNode) => ({
  appName: props.appName ? props.appName : DEFAULT_JALRAKSHAK_PROPS.appName,
  mapStyle: props.mapStyle,
  visState: props.visState,
  ...
  cloudProviders: props.cloudProviders
    ...
    : DEFAULT_JALRAKSHAK_PROPS.cloudProviders,
...
export const notificationPanelSelector = (props: JalrakshakGLProps) => ({
  removeNotification: props.uiStateActions.removeNotification,
  notifications: props.uiState.notifications
});

export const DEFAULT_JALRAKSHAK_PROPS = {
  mapStyles: [],
...
function JalrakshakFactory(
...
): React.ComponentType<JalrakshakGLBasicProps & { selector: (...args: any[]) => JalrakshakState }> {
  /** @typedef {import('./jalrakshak-gl').UnconnectedJalrakshakProps} JalrakshakProps */
  /** @augments React.Component<JalrakshakProps> */
  class JalrakshakGL extends Component<
    JalrakshakGLProps & { selector: (...args: any[]) => JalrakshakState },
    JalrakshakGLCompState
  > {
    ...
    componentDidMount() {
      ...
      this._loadMapStyle();
      if (typeof this.props.onJalrakshakInitialized === 'function') {
        this.props.onJalrakshakInitialized();
      }
    ...
    availableProviders = createSelector(
      (props: JalrakshakGLProps) => props.cloudProviders,
      providers =>
    ...
    localeMessagesSelector = createSelector(
      (props: JalrakshakGLProps) => props.localeMessages,
    ...
    render() {
      const {
        id = 'map',
        width = DEFAULT_JALRAKSHAK_PROPS.width,
        height = DEFAULT_JALRAKSHAK_PROPS.height,
        uiState,
        visState,
        // readOnly override
        readOnly,
  ...
  return jalrakshakGlConnect(
    mapStateToProps,
    makeMapDispatchToProps
  )(withTheme(JalrakshakGL)) as ReturnType<typeof JalrakshakFactory>;
}

export function mapStateToProps(state: JalrakshakState, props: JalrakshakGLProps) {
  return {
    ...props,
...
/**
 * Override default jalrakshak actions with user defined actions using the same key
 */
...
export default JalrakshakFactory;
