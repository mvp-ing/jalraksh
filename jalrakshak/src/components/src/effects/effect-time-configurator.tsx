...
import { MapState } from '@jalrakshak/types';

import { clamp } from '@jalrakshak/utils';
import {
  LIGHT_AND_SHADOW_EFFECT_TIME_MODES,
  LightAndShadowEffectTimeMode,
  DEFAULT_TIMEZONE
} from '@jalrakshak/constants';
import { mapStateLens } from '@jalrakshak/reducers';
  ...
  const EffectTimeConfigurator = ({
    ...
    const onTimeSliderChange = useCallback(
      value => {
        ...
        const newFormattedTime = `${hours < 10 ? `0${hours}` : hours}:${minutes < 10 ? `0${minutes}` : minutes
          }`;
