...
import JalrakshakContext from '../context';
...
): React.ComponentType<P & { selector: (...args: any[]) => JalrakshakState }> => {
  class WithConnectSelector extends Component<P & { selector: (...args: any[]) => JalrakshakState }> {
    static contextType = JalrakshakContext;
    ...
    render() {
      // @ts-ignore Argument of type 'Readonly<P & { selector: (...args: any[]) => JalrakshakState; }>' is not assignable to parameter of type 'never'
      const computedContext = this.contextSelector(this.props, this.context);
      return (
        <JalrakshakContext.Provider value={computedContext}>
