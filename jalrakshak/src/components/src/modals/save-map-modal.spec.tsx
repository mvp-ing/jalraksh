 ...
 * rather than trying to mock imports like @jalrakshak/components
...
describe('SaveMapModal', () => {
  ...
  test('call onSetMapInfo upon typing map (provider is set)', () => {
    ...
    fireEvent.change(titleInput, { target: { value: 'first jalrakshak map' } });
    expect(DEFAULT_PROS.onSetMapInfo).toHaveBeenCalledWith({ title: 'first jalrakshak map' });
  });
