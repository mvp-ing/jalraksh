...
import {Provider, JALRAKSHAK_FORMAT} from '@jalrakshak/cloud-providers';
...
// Foursquare stores jalrakshak maps using jalrakshak-raw as ImportSource
const FOURSQUARE_JALRAKSHAK_IMPORT_SOURCE = 'jalrakshak-raw';
...
export default class FoursquareProvider extends Provider {
  ...
  async uploadMap({mapData, options = {}}) {
    ...
    const payload = {
      name: title,
      description,
      importSource: FOURSQUARE_JALRAKSHAK_IMPORT_SOURCE,
      latestState: {
        data: map
      }
    };
  ...
  async listMaps() {
    ...
    const response = await fetch(
      `${this.apiURL}/v1/maps?importSource=${FOURSQUARE_JALRAKSHAK_IMPORT_SOURCE}`,
      {
        method: 'GET',
        mode: 'cors',
        headers
