...
import {createDataContainer} from '@jalrakshak/utils';
import {formatCsv} from '@jalrakshak/reducers';

const NAME = 'carto';
const DISPLAY_NAME = 'CARTO';
const NAMESPACE = 'jalrakshak';
  ...
  async uploadMap({mapData = {}, options = {}}) {
    try {
      ...
      if (overwrite) {
        ...
        const regex = /(?:^jalrakshak_)([a-z0-9]+)(?:.json$)/;
    ...
    if (error && error.message) {
      ...
      switch (error.message) {
        ...
        case (error.message.match(/relation "[a-zA-Z0-9_]+" does not exist/) || {}).input:
          ...
          break;
        case (
          error.message.match(/Failed to copy to jalrakshak_[a-zA-Z0-9_]+: Too many retries/) || {}
        ).input:
