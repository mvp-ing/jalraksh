...
import {aiAssistantReducer} from '@jalrakshak/ai-assistant';
import {EXPORT_MAP_FORMATS} from '@jalrakshak/constants';
import {processGeojson, processRowObject, processArrowTable} from '@jalrakshak/processors';
import jalrakshakGlReducer, {combinedUpdaters, uiStateUpdaters} from '@jalrakshak/reducers';
...
// import {getApplicationConfig, initApplicationConfig} from '@jalrakshak/utils';
// import jalrakshakGlDuckdbPlugin, {JalrakshakDuckDbTable, DuckDBWasmAdapter} from '@jalrakshak/duckdb';
...
// initialize jalrakshak demo-app with DuckDB plugin
/*
initApplicationConfig({
  // Custom UI for DuckDB
  plugins: [jalrakshakGlDuckdbPlugin],
  // async data ingestion to DuckDb
  table: JalrakshakDuckDbTable,
...
// combine app reducer and jalrakshakGl reducer
// to mimic the reducer state of jalrakshak website
const demoReducer = combineReducers({
  // mount jalrakshakGl reducer
  jalrakshakGl: jalrakshakGlReducer.initialState({
...
// this can be moved into a action and call jalrakshak action
/**
 * Used to load Jalrakshak.gl demo examples
 * @param state
 * @param action {map: resultset, config, map}
 * @returns {{app: {isMapLoading: boolean}, jalrakshakGl: {map: (state|*)}}}
 */
export const loadRemoteResourceSuccess = (state, action) => {
  ...
  if (dataUrl) {
    const table = getApplicationConfig().table ?? JalrakshakTable;
...
const loadRemoteDatasetProcessedSuccess = (state, action) => {
  ...
  const parsedConfig = config ? JalrakshakSchema.parseSavedConfig(config) : null;
  ...
  const jalrakshakGlInstance = combinedUpdaters.addDataToMapUpdater(
    state.jalrakshakGl.map, // "map" is the id of your jalrakshak instance
    {
      payload: {
        datasets,
        config: parsedConfig,
  ...
  return {
    ...
    jalrakshakGl: {
      ...state.jalrakshakGl, // in case you keep multiple instances
      map: jalrakshakGlInstance
    }
  };
};
  ...
  return {
    ...
    jalrakshakGl: {
      ...
      map: {
        ...
        uiState: uiStateUpdaters.addNotificationUpdater(state.jalrakshakGl.map.uiState, {
          payload: errorNote
        })
      }
    }
