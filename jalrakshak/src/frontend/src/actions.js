 ...
 * @param {string} [options.dataUrl] the URL to fetch data from, e.g. https://raw.githubusercontent.com/jalrakshak/jalrakshak-data/master/earthquakes/data.csv
 * @param {string} [options.configUrl] the URL string to fetch jalrakshak config from, e.g. https://raw.githubusercontent.com/jalrakshak/jalrakshak-data/master/earthquakes/config.json
 ...
 * @param {string} [jalrakshak] url to fetch the full data/config generated by jalrakshak
 * @returns {Function}
 */
export function loadSample(options, pushRoute = true) {
  return (dispatch, getState) => {
    ...
    // if the sample has a jalrakshak config file url we load it
    if (options.jalrakshak) {
      dispatch(loadRemoteMap({dataUrl: options.jalrakshak}));
    } else {
