...
const getThirdPartyLibraryAliases = useJalrakshakNodeModules => {
  const nodeModulesDir = useJalrakshakNodeModules ? NODE_MODULES_DIR : BASE_NODE_MODULES_DIR;

  const localSources = useJalrakshakNodeModules
    ? {
  ...
  return {
    ...
    // jalrakshak and loaders.gl need to use same apache-arrow
...
const config = {
  ...
  plugins: [
    ...
    replace({
      __PACKAGE_VERSION__: JalrakshakPackage.version,
...
function addAliases(externals, args) {
  ...
  if (useLocalAiAssistant) {
    ...
    resolveAlias['@jalrakshak/ai-assistant'] = join(SRC_DIR, 'ai-assistant/src');
  }

  // Add aliases for all jalrakshak workspaces (similar to webpack config)
  const workspaces = JalrakshakPackage.workspaces || [];
  workspaces.forEach(workspace => {
    // workspace example: "./src/types"
    const moduleName = workspace.split('/').pop();
    if (moduleName && moduleName !== 'frontend') { // exclude self
       resolveAlias[`@jalrakshak/${moduleName}`] = join(SRC_DIR, `${moduleName}/src`);
    }
  });

  // Explicitly alias @jalrakshak/duckdb/components to source to bypass package exports pointing to dist
  resolveAlias['@jalrakshak/duckdb/components'] = join(SRC_DIR, 'duckdb/src/components');


  // resolve deck.gl from local dir
  if (useLocalDeck || useRepoDeck) {
    // Load deck.gl from root node_modules
    // if env.deck_src Load deck.gl from deck.gl/modules/main/src folder parallel to jalrakshak
    ...
    // if env.deck_src Load @deck.gl modules from  deck.gl/modules folder parallel to jalrakshak
    ...
    ['luma.gl', 'probe.gl', 'loaders.gl'].forEach(name => {
      ...
      // if env.deck_src Load ${name} from deck.gl/node_modules folder parallel to jalrakshak
      ...
      // if env.deck_src Load @${name} modules from deck.gl/node_modules/@${name} folder parallel to jalrakshak`
  ...
  if (args.includes('--start')) {
    await esbuild
      ...
      .then(async ctx => {
        ...
        console.info(
          `jalrakshak demo app running at ${`http://localhost:${port}`}, press Ctrl+C to stop`
        );
