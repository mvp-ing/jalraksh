...
import { LayerClasses } from '@jalrakshak/layers';
import JalrakshakTable, { Datasets } from '@jalrakshak/table';
import { findDefaultLayer } from '@jalrakshak/reducers';
import { addLayer as addLayerAction } from '@jalrakshak/actions';
...
>({
  description: `Add a jalrakshak map layer from a dataset.
...
async function executeAddLayer(args, options): Promise<ExecuteAddLayerResult> {
  try {
    ...
    // check if dataset exists in jalrakshak
    ...
    if (colorBy) {
      ...
      // create jalrakshak's colorMap from uniqueValues and breaks
      const colorScale = colorType === 'breaks' ? 'custom' : 'customOrdinal';
      const colors = colorMap?.map(color => color.color);
      const jalrakshakColorMap = colorMap?.map(color => [color.value, color.color]);
      const colorRange = {
        name: 'color.customPalette',
        type: 'custom',
        category: 'Custom',
        colors,
        colorMap: jalrakshakColorMap
      };
...
export function guessDefaultLayer(dataset: JalrakshakTable, layerType: string) {
