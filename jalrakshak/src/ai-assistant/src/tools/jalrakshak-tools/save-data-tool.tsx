...
import { ProtoDataset } from '@jalrakshak/types';
...
export const saveToolResults = extendedTool({
  description:
    'Save tool results to jalrakshak. The tool includes: buffer, zipcode, county, state, isochrone, thiessenPolygons, mst, cartogram, etc.',
  parameters: z.object({
  ...
  execute: async (args: { datasetNames: string[] }) => {
    try {
      ...
      // try to process the merged result using jalrakshak processor
      ...
      return {
        llmResult: {
          ...
          details: `Successfully save dataset: ${datasetName} in jalrakshak`
        },
        additionalData: {
          parsedData
        }
      ...
      return {
        llmResult: {
          success: false,
          details: `Can not save data to jalrakshak, ${error}`
        }
      };
    }
  },
