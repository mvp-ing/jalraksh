...
import { addDataToMap, addLayer } from '@jalrakshak/actions';
import { LisaAdditionalData } from '@openassistant/geoda';
import { saveAsDataset } from './utils';
import { guessDefaultLayer } from './jalrakshak-tools/layer-creation-tool';
import { State } from '../components/ai-assistant-manager';

/**
 * Use LisaToolComponent adding a customized Lisa map layer to jalrakshak
 */
export function LisaToolComponent(props: LisaAdditionalData) {
  ...
  const layers = useSelector((state: State) => state.demo.jalrakshakGl.map.visState.layers);
