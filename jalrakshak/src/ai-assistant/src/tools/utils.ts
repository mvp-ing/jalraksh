...
import {Layer, VectorTileLayer} from '@jalrakshak/layers';
import {Datasets, JalrakshakTable} from '@jalrakshak/table';
import {SpatialJoinGeometries} from '@openassistant/geoda';
import {ALL_FIELD_TYPES, LAYER_TYPES} from '@jalrakshak/constants';
import {Field, ProtoDataset, ProtoDatasetField} from '@jalrakshak/types';
import {processFileData} from '@jalrakshak/processors';
 ...
 * @param datasets The jalrakshak datasets
 * @param layers The jalrakshak layers
...
export function getDatasetContext(datasets?: Datasets, layers?: Layer[]) {
  ...
  const dataMeta = Object.values(datasets).map((dataset: JalrakshakTable) => ({
...
function getFeaturesFromVectorTile(leftDataset: JalrakshakTable, layers: Layer[]) {
