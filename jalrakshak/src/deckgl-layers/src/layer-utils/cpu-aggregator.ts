...
import {aggregate} from '@jalrakshak/utils';
import {AGGREGATION_TYPES, SCALE_FUNC} from '@jalrakshak/constants';
import {RGBAColor} from '@jalrakshak/types';
...
export function getValueFunc(aggregation, accessor) {
  ...
  const jalrakshakOp = DECK_AGGREGATION_MAP[op];

  return pts => aggregate(pts.map(accessor), jalrakshakOp);
}
