...
import {ProtoDatasetField} from '@jalrakshak/types';
import {ALL_FIELD_TYPES, GUIDES_FILE_FORMAT_DOC} from '@jalrakshak/constants';
...
/*
 * Process uploaded exported Jalrakshakgl json map.
...
export async function processJalrakshakglJSONforDuckDb(
  jalrakshakJson
): Promise<ReturnType<typeof processJalrakshakglJSON>> {
  const res = processJalrakshakglJSON(jalrakshakJson);
