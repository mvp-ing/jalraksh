...
import { Button, FileDrop, IconButton, Icons, LoadingSpinner, Tooltip } from '@jalrakshak/components';
import { arrowSchemaToFields } from '@jalrakshak/processors';
import { sidePanelBg, panelBorderColor } from '@jalrakshak/styles';
import { isAppleDevice, getApplicationConfig } from '@jalrakshak/utils';
...
export const SqlPanel: React.FC<SqlPanelProps> = ({ initialSql = '' }) => {
  ...
  const runQuery = useCallback(async () => {
    ...
    try {
      ...
      const tempTableName = 'temp_jalrakshak_table';
      ...
      for (const statement of sqlStatements) {
        ...
        if (isLastQuery && (await checkIsSelectQuery(connection, statement))) {
          ...
          const adjustedQuery = castDuckDBTypesForJalrakshak(tempTableName, duckDbColumns);
  ...
  const onAddResultToMap = useCallback(() => {
    ...
    const jalrakshakFields = arrowSchemaToFields(result.table, result.tableDuckDBTypes);

    const datasetToAdd = {
      data: {
        fields: jalrakshakFields,
