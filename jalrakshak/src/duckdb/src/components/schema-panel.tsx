...
import { LoadingSpinner, Icons } from '@jalrakshak/components';
import { arrowSchemaToFields } from '@jalrakshak/processors';
import { VisState } from '@jalrakshak/schemas';
import { getApplicationConfig, DatabaseConnection } from '@jalrakshak/utils';
...
async function getColumnSchema(connection: DatabaseConnection, tableName: string) {
  ...
  const jalrakshakFields = arrowSchemaToFields(columnResult, getDuckDBColumnTypesMap(columnDescribe));
    ...
    children: columnResult.schema.fields.map((field, fieldIndex) => {
      return {
        ...
        object: {
          ...
          fieldType: jalrakshakFields[fieldIndex].type
        }
      };
    })
  };
...
export const SchemaPanel = ({
  ...
  const datasets = useSelector((state: State) => state?.demo?.jalrakshakGl?.map?.visState.datasets);
