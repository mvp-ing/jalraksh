...
import {
  ...
  isRowObject,
  processArrowBatches
} from '@jalrakshak/processors';
import {JalrakshakTable} from '@jalrakshak/table';
import {Field} from '@jalrakshak/types';
import {
...
import {
  processCsvRowObject,
  processGeojson,
  processJalrakshakglJSONforDuckDb,
  ProcessorResult
...
/**
 Default column name for processed geojson in Jalrakshak.
 Use this name to rename default 'geom' column from DuckDb
 in order to support Jalrakshak maps saved before introduction of DuckDb plugin.
 */
const JALRAKSHAK_GEOM_FROM_GEOJSON_COLUMN = '_geojson';

/**
 * Names of columns that most likely contain binary wkb geometry
 */
const SUGGESTED_GEOM_COLUMNS = {
  [JALRAKSHAK_GEOM_FROM_GEOJSON_COLUMN]: GEOARROW_EXTENSIONS.WKB,
...
export class JalrakshakDuckDbTable extends JalrakshakTable {
  ...
  async importRowData({data, db, c}: ImportDataToDuckProps): Promise<void> {
    try {
      ...
      // DATASET_FORMATS.jalrakshak loads data as any[][] instead of [{}]
  ...
  async importGeoJsonData({data, db, c}: ImportDataToDuckProps): Promise<ImportDataToDuckResult> {
    try {
      ...
      const createTableSql = `
        ...
        ALTER TABLE '${this.label}' RENAME '${DUCKDB_WKB_COLUMN}' TO '${JALRAKSHAK_GEOM_FROM_GEOJSON_COLUMN}';
      `;
    ...
    return {
      ...
      geoarrowMetadata: {[JALRAKSHAK_GEOM_FROM_GEOJSON_COLUMN]: GEOARROW_EXTENSIONS.WKB}
    };
  }
  ...
  protected async createTableAndGetArrow(data): Promise<{fields: any[]; cols: arrow.Vector[]}> {
    ...
    if (!format) {
      // format is missing when we load Jalrakshak.gl examples
    ...
    try {
      ...
      const adjustedQuery = castDuckDBTypesForJalrakshak(tableName, duckDbColumns);
  ...
  static getFileProcessor = function (data: any, inputFormat?: string) {
    ...
    } else if (inputFormat === DATASET_FORMATS.jalrakshak || isJalrakshakMap(data)) {
      format = DATASET_FORMATS.jalrakshak;
      processor = processJalrakshakglJSONforDuckDb;
