...
import {ALL_FIELD_TYPES, DATASET_FORMATS} from '@jalrakshak/constants';
import {ProtoDataset, RGBColor, JsonObject} from '@jalrakshak/types';
import {JalrakshakTable} from '@jalrakshak/table';
import {VERSIONS} from './versions';
import Schema from './schema';
import {getFieldsFromData, getSampleForTypeAnalyze} from '@jalrakshak/common-utils';
import {ArrowDataContainer, DataContainerInterface, FIELD_DISPLAY_FORMAT} from '@jalrakshak/utils';
 ...
 * 1) Arrow tables can store Timestamps as BigInts, so convert numbers to ISOStrings compatible with Jalrakshak.gl's TIMESTAMP.
 ...
 * @param fields The array of fields from a Jalrakshak table.
 * @returns The transformed fields array with GeoArrow types updated to GeoJSON.
 */
const getFieldsForSaving = (fields: JalrakshakTable['fields']) => {
...
export class DatasetSchema extends Schema {
  ...
  save(dataset: JalrakshakTable): SavedDatasetV1['data'] {
