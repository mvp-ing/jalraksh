...
import {Feature, Field as JalrakshakField, JalrakshakLayer} from '@jalrakshak/types';
import {clamp, formatNumberByStep, getNumericStepSize, timeToUnixMilli} from '@jalrakshak/utils';

import {
  FilterProps,
  NumericFieldFilterProps,
  StringFieldFilterProps,
  default as JalrakshakDataset
} from '../jalrakshak-table';

export function isTileDataset(dataset: JalrakshakDataset | {type: string}): boolean {
 ...
 * @returns Metadata in Jalrakshak-friendly format.
 */
 ...
 */
function mergeAttributeDomain(field: JalrakshakField, attribute: TippecanoeLayerAttribute): void {
  switch (field.type) {
    case ALL_FIELD_TYPES.real:
    case ALL_FIELD_TYPES.integer: {
...
function attributeTypeToFieldType(aType?: string): {type: string; analyzerType: string} {
  ...
  if (!type || !attrTypeMap[type]) {
    Console.warn(
      `cannot convert attribute type ${type} to jalrakshak data type, use string by default`
    );
 ...
 */
export function matchDatasetType(dataset: JalrakshakDataset, layer: JalrakshakLayer): boolean {
...
/**
 * Converts tile features into Jalrakshak fields.
 * @param tile The tile object containing features.
 * @returns An array of Jalrakshak fields derived from the tile's features.
 */
const tileToFields = (tile: {features: Feature[]}): JalrakshakField[] => {
