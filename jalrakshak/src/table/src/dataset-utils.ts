...
import {ProtoDataset, RGBColor} from '@jalrakshak/types';
...
export function createNewDataEntry(
  ...
  if (info && info.id && datasets[info.id]) {
    // get jalrakshakTable from datasets
    const jalrakshakTable = datasets[info.id];
    // update the data in jalrakshakTable
    return UPDATE_TABLE_TASK({table: jalrakshakTable, data: validatedData});
  }
