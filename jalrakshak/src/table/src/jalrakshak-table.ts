...
import {generateHashId, notNullorUndefined} from '@jalrakshak/common-utils';

// TODO isolate layer type, depends on @jalrakshak/layers
type Layer = any;
...
class JalrakshakTable<F extends Field = Field> {
  readonly id: string;
  ...
  constructor({
    ...
    // TODO - what to do if validation fails? Can jalrakshak handle exceptions?
   ...
   * @param opt
   */
  filterTable(filters: Filter[], layers: Layer[], opt?: FilterDatasetOpt): JalrakshakTable<Field> {
   ...
   * @param layers
   */
  filterTableCPU(filters: Filter[], layers: Layer[]): JalrakshakTable<Field> {
    const opt = {
      cpuOnly: true,
      ignoreDomain: true
    };
...
export type Datasets = {
  [key: string]: JalrakshakTable<Field>;
};
...
export function sortDatasetByColumn(
  dataset: JalrakshakTable<Field>,
  column: string,
  mode?: string
): JalrakshakTable<Field> {
...
export function pinTableColumns(dataset: JalrakshakTable<Field>, column: string): JalrakshakTable<Field> {
...
export function copyTable(original: JalrakshakTable<Field>): JalrakshakTable<Field> {
...
export function copyTableAndUpdate(
  original: JalrakshakTable<Field>,
  options: Partial<JalrakshakTable<Field>> = {}
): JalrakshakTable<Field> {
...
export default JalrakshakTable;
