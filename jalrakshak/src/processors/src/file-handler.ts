...
import {DATASET_FORMATS} from '@jalrakshak/constants';
import {AddDataToMapPayload, Feature, LoadedMap, ProcessorResult} from '@jalrakshak/types';
...
import {FileCacheItem, ValidJalrakshakMap} from './types';
...
export function isJalrakshakMap(json: unknown): json is ValidJalrakshakMap {
  return Boolean(
    isPlainObject(json) &&
      json.datasets &&
      json.config &&
...
export async function processFileData({
  ...
  if (typeof table.getFileProcessor === 'function') {
    ...
    } else if (isJalrakshakMap(data)) {
      format = DATASET_FORMATS.jalrakshak;
      processor = processJalrakshakglJSON;
...
export function filesToDataPayload(fileCache: FileCacheItem[]): AddDataToMapPayload[] {
  // seperate out files which could be a single datasets. or a jalrakshak map json
  const collection = fileCache.reduce<{
    datasets: FileCacheItem[];
    jalrakshakMaps: AddDataToMapPayload[];
  }>(
    (accu, file) => {
      const {data, info} = file;
      if (info?.format === DATASET_FORMATS.jalrakshak) {
        // if file contains a single jalrakshak map dataset & config
        accu.jalrakshakMaps.push({
          ...data,
          options: {
    ...
    {datasets: [], jalrakshakMaps: []}
  );

  // add jalrakshak map first with config
  // add datasets later in one add data call
  return collection.jalrakshakMaps.concat({datasets: collection.datasets});
}
