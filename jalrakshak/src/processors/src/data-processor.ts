...
import {ProcessorResult, Field} from '@jalrakshak/types';
import {
...
import {
  ...
  notNullorUndefined,
  toArray
} from '@jalrakshak/common-utils';
import {JalrakshakSchema, ParsedDataset, SavedMap, LoadedMap} from '@jalrakshak/schemas';
 ...
 * @public
 * @example
 * import {processCsvData} from '@jalrakshak/processors';
 *
 ...
 * import {processRowObject} from '@jalrakshak/processors';
 *
 * const data = [
 ...
 * @example
 * import {addDataToMap} from '@jalrakshak/actions';
 * import {processGeojson} from '@jalrakshak/processors';
 *
 * const geojson = {
...
/**
 * Process saved jalrakshak json to be pass to [`addDataToMap`](../actions/actions.md#adddatatomap).
 ...
 * @example
 * import {addDataToMap} from '@jalrakshak/actions';
 * import {processJalrakshakglJSON} from '@jalrakshak/processors';
 *
 * dispatch(addDataToMap(processJalrakshakglJSON(jalrakshakGlJson)));
 */
export function processJalrakshakglJSON(rawData: SavedMap, schema = JalrakshakSchema): LoadedMap | null {
  return rawData ? schema.load(rawData.datasets, rawData.config) : null;
}

/**
 * Parse a single or an array of datasets saved using jalrakshak schema
 * @param rawData
 * @param schema
 */
export function processJalrakshakglDataset(
  rawData: object | object[],
  schema = JalrakshakSchema
...
/**
 * Converts an Apache Arrow table schema into an array of Jalrakshak.gl field objects.
 ...
 * @returns An array of field objects suitable for Jalrakshak.gl.
 */
export function arrowSchemaToFields(
  ...
  const jalrakshakFields = getFieldsFromData(sample, headerRow);
  ...
  return table.schema.fields.map((field: arrow.Field, fieldIndex: number) => {
    ...
    const jalrakshakField = jalrakshakFields[fieldIndex];
    ...
    if (fieldTypeSuggestion === 'JSON') {
      ...
      (jalrakshakField.analyzerType === AnalyzerDATA_TYPES.GEOMETRY ||
        jalrakshakField.analyzerType === AnalyzerDATA_TYPES.GEOMETRY_FROM_STRING)
    ) {
      // When wkb/wkt was saved as varchar in DuckDB
      type = jalrakshakField.type;
      analyzerType = jalrakshakField.analyzerType;
      format = jalrakshakField.format;
    } else if (fieldTypeSuggestion === 'VARCHAR' && jalrakshakField.type === ALL_FIELD_TYPES.h3) {
      // when jalrakshak detected h3 column using getFieldsFromData(), set type to h3 and analyzerType to H3
      type = ALL_FIELD_TYPES.h3;
      analyzerType = jalrakshakField.analyzerType;
    } else {
      // TODO should we use Jalrakshak getFieldsFromData instead
      // of arrowDataTypeToFieldType for all fields?
      if (jalrakshakField.type === ALL_FIELD_TYPES.timestamp) {
        type = jalrakshakField.type;
        analyzerType = jalrakshakField.analyzerType;
        format = jalrakshakField.format;
      }
...
export const DATASET_HANDLERS = {
  ...
  [DATASET_FORMATS.jalrakshak]: processJalrakshakglDataset
};
...
export const Processors: {
  ...
  processJalrakshakglJSON: typeof processJalrakshakglJSON;
  processJalrakshakglDataset: typeof processJalrakshakglDataset;
  ...
  processArrowBatches,
  processRowObject,
  processJalrakshakglJSON,
  processJalrakshakglDataset,
  analyzerTypeToFieldType,
  getFieldsFromData,
