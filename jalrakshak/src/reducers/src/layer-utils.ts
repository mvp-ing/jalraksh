...
import {GEOCODER_LAYER_ID, LIGHT_AND_SHADOW_EFFECT} from '@jalrakshak/constants';
...
import {
  ...
  OVERLAY_TYPE_CONST,
  getEditorLayer
} from '@jalrakshak/layers';

import JalrakshakTable from '@jalrakshak/table';
import {VisState} from '@jalrakshak/schemas';
import {isFunction, getMapLayersFromSplitMaps, DataRow} from '@jalrakshak/utils';
import {arrayMove} from '@jalrakshak/common-utils';

import {ThreeDBuildingLayer} from '@jalrakshak/deckgl-layers';
 ...
 */
export function findDefaultLayer(dataset: JalrakshakTable, layerClasses: LayerClassesType): Layer[] {
  if (!dataset) {
    return [];
  }
...
export function getLayerHoverProp({
  ...
  if (interactionConfig.tooltip.enabled && hoverInfo && hoverInfo.picked) {
    ...
    // deckgl layer to jalrakshak-gl layer
