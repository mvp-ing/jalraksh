...
import {getApplicationConfig, getError, isPlainObject} from '@jalrakshak/utils';
import {generateHashId, toArray} from '@jalrakshak/common-utils';
import {
  EXPORT_FILE_TO_CLOUD_TASK,
  ACTION_TASK,
  DELAY_TASK,
  LOAD_CLOUD_MAP_TASK
} from '@jalrakshak/tasks';
import {
  exportFileSuccess,
  exportFileError,
  postSaveLoadSuccess,
...
import {AddDataToMapPayload, ExportFileToCloudPayload} from '@jalrakshak/types';

import {FILE_CONFLICT_MSG, MapListItem} from '@jalrakshak/cloud-providers';
import {DATASET_HANDLERS} from '@jalrakshak/processors';
...
/**
 * This method will export the current jalrakshak config file to the chosen cloud proder
...
function getDatasetHandler(format) {
  ...
  const TableClass = getApplicationConfig().table ?? JalrakshakTable;
...
async function parseLoadMapResponseTask({
  ...
  if (
    ...
    processorMethod !== DATASET_HANDLERS[DATASET_FORMATS.jalrakshak]
  ) {
    // plugin table provides processor for jalrakshak map, not single dataset with allData
    ...
    parsedDatasets = await Promise.all(
      datasets.map(async ds => {
        if (format === DATASET_FORMATS.jalrakshak) {
