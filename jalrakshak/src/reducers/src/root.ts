...
import {coreReducerFactory, JalrakshakState} from './core';

type JalrakshakStateMap = {
  [id: string]: Partial<JalrakshakState>;
};
...
export function provideInitialState(initialState, extraReducers?) {
  ...
  const handleRegisterEntry = (
    ...
    return {
      // register entry to jalrakshak passing in mapbox config to mapStyle
      ...state,
      [id]: coreReducer(
        previousState,
        jalrakshakGlInit({mapboxApiAccessToken, mapboxApiUrl, mapStylesReplaceDefault, initialUiState})
      )
    };
  };
  ...
  return (state = initialCoreState, action) => {
    ...
    return handleActions<JalrakshakStateMap, CombineRegisterUpdateActions>(
      handlers,
      initialCoreState
    )(state, action);
  };
...
const _jalrakshakGlReducer = provideInitialState(initialCoreState);
...
function decorate(target, savedInitialState = {}) {
  ...
  /**
   * Returns a jalrakshak reducer that will also pass each action through additional reducers spiecified.
   ...
   * `.plugin` is only meant to be called once when mounting the jalrakshakGlReducer to the store.
   * **Note** This is an advanced option to give you more freedom to modify the internal state of the jalrakshak instance.
   * You should only use this to adding additional actions instead of replacing default actions.
   *
   * @mixin jalrakshakGlReducer.plugin
   * @memberof jalrakshakGlReducer
   ...
   * @public
   * @example
   * const myJalrakshakReducer = jalrakshakGlReducer
   *  .plugin({
   *    // 1. as reducer map
   ...
   * @mixin jalrakshakGlReducer.initialState
   * @memberof jalrakshakGlReducer
...
/**
 * Jalrakshak.gl reducer to be mounted to your store. You can mount `jalrakshakGlReducer` at property `jalrakshakGl`, if you choose
 * to mount it at another address e.g. `foo` you will need to specify it when you mount `Jalrakshak` component in your app with `getState: state => state.foo`
 * @public
 * @example
 ...
 */
const jalrakshakGlReducer = decorate(_jalrakshakGlReducer);
export default jalrakshakGlReducer;
