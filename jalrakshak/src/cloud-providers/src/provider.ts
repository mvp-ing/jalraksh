...
import {MapData, ExportFileOptions, Millisecond, SavedMap} from '@jalrakshak/types';
...
const ICON = Upload;
export const JALRAKSHAK_FORMAT = 'jalrakshak';
...
export default class Provider {
  ...
  /**
   * This method is called by jalrakshak demo app to pushes a new location to history, becoming the current location.
   * @returns mapUrl
   * @public
   */
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  /**
   * This method will be called to upload map for saving and sharing. Jalrakshak.gl will package map data, config, title, description and thumbnail for upload to storage.
   ...
   * @param {boolean} [param.options.isPublic] - whether user wish to share the map with others. if isPublic is truthy, jalrakshak will call this.getShareUrl() to display an URL they can share with others
   * @public
   */
  async uploadMap({
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
   ...
   *        id: 'a',
   *        title: 'My map',
   *        description: 'My first jalrakshak map',
   *        imageUrl: 'http://',
   ...
   * to inform how jalrakshak should process your data object, pass in `format`
   * @typedef {Object} MapResponse
   * @property {Object} map
   * @property {Array<Object>} map.datasets
   * @property {Object} map.config
   * @property {Object} map.info
   * @property {string} format - one of 'csv': csv file string, 'geojson': geojson object, 'row': row object, 'jalrakshak': datasets array saved using JalrakshakSchema.save
   * @public
   */
}
