...
/**
 * Add data to jalrakshak reducer, prepare map with preset configuration if config is passed.
 * Jalrakshak.gl provides a handy set of utils to parse data from different formats to the `data` object required in dataset. You rarely need to manually format the data obejct.
 *
 * Use `JalrakshakSchema.getConfigToSave` to generate a json blob of the currents instance config.
 * The config object value will always have higher precedence than the options properties.
 *
 * Jalrakshak.gl uses `dataId` in the config to match with loaded dataset. If you pass a config object, you need
 ...
 * @param {boolean} data.options.centerMap `default: true` if `centerMap` is set to `true` jalrakshak will
 ...
 * @param {Object} data.config this object will contain the full jalrakshak instance configuration {mapState, mapStyle, visState}
 * @public
 * @example
 *
 * // app.js
...
/**
 * Pass config to jalrakshak instance, prepare the state with preset configs.
 * Calling `JalrakshakSchema.parseSavedConfig` to convert saved config before passing it in is required.
 *
 ...
 *
 * It is also useful when you want to prepare the jalrakshak instance with some preset layer and filter settings.
 * **Note** Sequence is important, `receiveMapConfig` needs to be called __before__ data is loaded. Currently jalrakshak doesn't allow calling `receiveMapConfig` after data is loaded.
 ...
 * @param {boolean} options.centerMap `default: true` if `centerMap` is set to `true` jalrakshak will
 ...
 * @public
 * @example
 * import {receiveMapConfig} from '@jalrakshak/actions';
 * import JalrakshakSchema from '@jalrakshak/schemas';
 *
 * const parsedConfig = JalrakshakSchema.parseSavedConfig(config);
...
export type JalrakshakInitPayload = {
...
/**
 * Initialize jalrakshak reducer. It is used to pass in `mapboxApiAccessToken` to `mapStyle` reducer.
 * @memberof main
 ...
 */
// @ts-expect-error
export const jalrakshakGlInit: (options?: JalrakshakInitPayload) => {
  type: typeof ActionTypes.INIT;
  payload: JalrakshakInitPayload;
} = createAction(ActionTypes.INIT, (payload: JalrakshakInitPayload) => ({payload}));
 ...
 */
/**
 * Main jalrakshak actions, these actions handles loading data and config into jalrakshak reducer. These actions
