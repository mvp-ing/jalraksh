...
import {FileCacheItem} from '@jalrakshak/processors';
import {Layer, LayerBaseConfig} from '@jalrakshak/layers';
 ...
 * @param options.centerMap `default: true` if `centerMap` is set to `true` jalrakshak will
 * place the map view within the data points boundaries
 * @param options.readOnly `default: false` if `readOnly` is set to `true`
 * the left setting panel will be hidden
 * @param config this object will contain the full jalrakshak instance configuration {mapState, mapStyle, visState}
 * @returns action
 * @public
 */
 ...
 * @public
 * @example
 * import {setMapMode} from '@jalrakshak/actions';
 * import {EDITOR_MODES} from '@jalrakshak/constants';
 *
...
export type CreateNewDatasetSuccessPayload = {
  results: (PromiseFulfilledResult<JalrakshakTable> | PromiseRejectedResult)[];
