...
import {DatasetType, WMSDatasetMetadata, LAYER_TYPES} from '@jalrakshak/constants';
import {WMSLayer as DeckWMSLayer} from '@jalrakshak/deckgl-layers';
import {JalrakshakTable as JalrakshakDataset} from '@jalrakshak/table';
import {
  AnimationConfig,
  Field,
  LayerBaseConfig,
...
export default class WMSLayer extends AbstractTileLayer<WMSTile, any[]> {
  ...
  updateLayerMeta(dataset: JalrakshakDataset): void {
    if (dataset.type !== DatasetType.WMS_TILE) {
      return;
    }
