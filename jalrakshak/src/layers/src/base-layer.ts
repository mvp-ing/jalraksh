...
import {GeoArrowTextLayer} from '@jalrakshak/deckgl-arrow-layers';
...
import {generateHashId, toArray, notNullorUndefined} from '@jalrakshak/common-utils';
import {Datasets, GpuFilter, JalrakshakTable} from '@jalrakshak/table';
import {
  AggregatedBin,
  ColorRange,
  ColorUI,
...
class Layer implements JalrakshakLayer {
  id: string;
   ...
   * @param param.dataAccessor Access jalrakshak layer data from deck.gl layer
  ...
  getDataUpdateTriggers({filteredIndex, id, dataContainer}: JalrakshakTable): any {
  ...
  updateLayerVisualChannel(dataset: JalrakshakTable, channel: string) {
    const visualChannel = this.visualChannels[channel];
    this.validateVisualChannel(channel);
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  calculateDataAttribute(jalrakshakTable: JalrakshakTable, getPosition): any {
    // implemented in subclasses
    return [];
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  updateLayerMeta(dataset: JalrakshakTable, getPosition) {
    // implemented in subclasses
  }
