...
import {DatasetType, PMTilesType, LAYER_TYPES} from '@jalrakshak/constants';
import {RasterLayer, RasterMeshLayer} from '@jalrakshak/deckgl-layers';
import {
  LayerBaseConfig,
  LayerColorConfig,
  LayerHeightConfig,
...
import {Datasets, JalrakshakTable as JalrakshakDataset, VectorTileMetadata} from '@jalrakshak/table';
...
import {default as JalrakshakLayer, LayerBaseConfigPartial} from '../base-layer';
...
export default class RasterTileLayer extends JalrakshakLayer {
  ...
  formatLayerData(datasets: Datasets, oldLayerData?: any) {
    ...
    if (triggerChanged && triggerChanged.getMeta) {
      this.updateLayerMeta(dataset as JalrakshakRasterDataset);
    }
  ...
  updateLayerMeta(dataset: JalrakshakRasterDataset): void {
    if (dataset.type !== DatasetType.RASTER_TILE) {
      return;
    }
   ...
   */
  setInitialLayerConfig(dataset: JalrakshakDataset): RasterTileLayer {
