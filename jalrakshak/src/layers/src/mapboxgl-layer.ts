...
import {Merge, LayerColumn} from '@jalrakshak/types';
...
class MapboxLayerGL extends Layer {
  ...
  getDataUpdateTriggers({filteredIndex, gpuFilter, id}: JalrakshakTable): any {
  ...
  calculateDataAttribute({dataContainer, filteredIndex, gpuFilter}: JalrakshakTable, getPosition) {
