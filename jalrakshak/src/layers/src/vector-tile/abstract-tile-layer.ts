...
import {SCALE_TYPES, ALL_FIELD_TYPES, LAYER_VIS_CONFIGS} from '@jalrakshak/constants';
import {
  isTileDataset,
...
import {findDefaultColorField, DataContainerInterface} from '@jalrakshak/utils';
...
> extends JalrakshakLayer {
  ...
  ): (field: JalrakshakField, datum: T extends Iterable<infer V> ? V : never) => string | number | null;
  ...
  setInitialLayerConfig(dataset: JalrakshakDataset): AbstractTileLayer<T, I> {
  ...
  updateLayerDomain(datasets: JalrakshakDatasets, newFilter?: Filter): AbstractTileLayer<T, I> {
  ...
  updateLayerVisualChannel(dataset: JalrakshakDataset, channel: string): void {
