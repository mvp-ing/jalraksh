...
import {
  getTileUrl,
  JalrakshakTable as JalrakshakDataset,
  Datasets as JalrakshakDatasets,
  GpuFilter,
  VectorTileMetadata
} from '@jalrakshak/table';
import {
  AnimationConfig,
  Field as JalrakshakField,
  LayerColorConfig,
  LayerHeightConfig,
  Merge,
  MapState,
...
export default class VectorTileLayer extends AbstractTileLayer<VectorTile, Feature[]> {
  ...
  static findDefaultLayerProps(dataset: JalrakshakDataset): FindDefaultLayerPropsReturnValue {
  ...
  get layerIcon(): JalrakshakLayer['layerIcon'] {
  ...
  getScaleOptions(channelKey: string): string[] {
    let options = JalrakshakLayer.prototype.getScaleOptions.call(this, channelKey);
  ...
  accessRowValue(
    field?: JalrakshakField,
    indexKey?: number | null
  ): (field: JalrakshakField, datum: Feature) => number | null {
    // if is indexed field
  ...
  updateLayerMeta(dataset: JalrakshakDataset, datasets: JalrakshakDatasets): void {
    if (dataset.type !== DatasetType.VECTOR_TILE) {
      return;
    }
  ...
  renderLayer(opts: VectorTileLayerRenderOptions): DeckLayer[] {
    ...
    const colorField = this.config.colorField as JalrakshakField;
    const heightField = this.config.heightField as JalrakshakField;
    const strokeColorField = this.config.strokeColorField as JalrakshakField;
    const sizeField = this.config.sizeField as JalrakshakField;
    const radiusField = this.config.radiusField as JalrakshakField;
