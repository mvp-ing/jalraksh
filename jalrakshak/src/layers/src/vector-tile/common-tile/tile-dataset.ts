...
import {Field as JalrakshakField} from '@jalrakshak/types';
import {quickInsertionSort} from '@jalrakshak/utils';
...
export default class TileDataset<T, I extends Iterable<any> = T extends Iterable<any> ? T : never> {
   ...
   */
  getExtent(field: JalrakshakField): [number, number] {
   ...
   */
  getQuantileSample(field: JalrakshakField, minRowCount = 1000): number[] {
   ...
   */
  getUniqueValues(field: JalrakshakField): Datum[] {
    const {getRowValue, getIterable} = this.accessors;
    const accessor = getRowValue(field);
