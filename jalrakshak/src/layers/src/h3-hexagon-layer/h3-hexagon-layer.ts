...
import {EnhancedColumnLayer} from '@jalrakshak/deckgl-layers';
import {
  getCentroid,
  idToPolygonGeo,
  h3IsValid,
...
import {findDefaultColorField, DataContainerInterface, createDataContainer} from '@jalrakshak/utils';
...
import {Datasets, JalrakshakTable} from '@jalrakshak/table';
...
export default class HexagonIdLayer extends Layer {
  ...
  constructor(props) {
    ...
    this.getPositionAccessor = (dataContainer: DataContainerInterface, dataset?: JalrakshakTable) => {
  ...
  static findDefaultLayerProps({fields = [], dataContainer, label}: JalrakshakTable) {
  ...
  calculateDataAttribute({filteredIndex}: JalrakshakTable, getHexId) {
  ...
  updateLayerMeta(dataset: JalrakshakTable, getHexId) {
