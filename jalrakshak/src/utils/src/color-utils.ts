...
import {JALRAKSHAK_COLOR_PALETTES, PALETTE_TYPES, SCALE_TYPES} from '@jalrakshak/constants';
...
export function updateColorRangeByMatchingPalette(
  ...
  const matchingPalette = JALRAKSHAK_COLOR_PALETTES.filter(
    palette =>
      // palette match type
...
export function colorRangeBackwardCompatibility(colorRange: ColorRange): ColorRange {
  ...
  if (trimName) {
    const matchingPalette = JALRAKSHAK_COLOR_PALETTES.find(p => p.name === trimName);
    if (matchingPalette) {
      return {
        ...colorRange,
        name: trimName,
