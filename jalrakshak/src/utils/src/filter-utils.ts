...
import {generateHashId, toArray, notNullorUndefined, getCentroid} from '@jalrakshak/common-utils';
...
import {JalrakshakTableModel} from './types';
...
// TODO isolate types - depends on @jalrakshak/schemas
type VisState = any;
 ...
 */
export function validatePolygonFilter<K extends JalrakshakTableModel<K, L>, L extends {id: string}>(
  dataset: K,
 ...
 * @return - {filter, dataset}
 */
export function validateFilter<K extends JalrakshakTableModel<K, L>, L>(
 ...
 */
export function validateFilterWithData<K extends JalrakshakTableModel<K, L>, L>(
  datasets: Record<string, K>,
  datasetId: string,
  filter: ParsedFilter,
  layers: L[]
...
export function getColumnFilterProps<K extends JalrakshakTableModel<K, L>, L>(
  filter: Filter,
  dataset: K
...
export function updateFilterPlot<K extends JalrakshakTableModel<K, any>>(
...
export function applyFiltersToDatasets<
  K extends JalrakshakTableModel<K, L>,
 ...
 */
export function applyFilterFieldName<K extends JalrakshakTableModel<K, L>, L>(
  filter: Filter,
  datasets: Record<string, K>,
  datasetId: string,
  fieldName: string,
...
export function mergeFilterDomain(
  filter: Filter,
  datasets: Record<string, JalrakshakTableModel<any, any>>
 ...
 */
interface StateType<K extends JalrakshakTableModel<K, L>, L> {
  layers: L[];
  filters: Filter[];
  datasets: {[id: string]: K};
}

export function filterDatasetCPU<T extends StateType<K, L>, K extends JalrakshakTableModel<K, L>, L>(
  state: T,
  dataId: string
): T {
